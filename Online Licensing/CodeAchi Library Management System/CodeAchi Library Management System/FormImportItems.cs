...
namespace CodeAchi_Library_Management_System
{
    public partial class FormImportItems : Form
    {
        ...
        private void btnImport_Click(object sender, EventArgs e)
        {
            try
            {
                if (!Directory.Exists(Properties.Settings.Default.databasePath + @"\CatlogImage"))
                {
                    Directory.CreateDirectory(Properties.Settings.Default.databasePath + @"\CatlogImage");
                }
                ...
                else
                {
                            ...
                            else
                            {
                                ...
                                if (globalVarLms.sqliteData)
                                {
                                    ...
                                    if (File.Exists(txtbImage.Text + @"\" + imgPath))
                                    {
                                        ...
                                        imgName = Properties.Settings.Default.databasePath + @"\CatlogImage\" + itemId + ".jpg";
                                        if (!File.Exists(imgName))
                                        {
                                            itemImage.Save(imgName, System.Drawing.Imaging.ImageFormat.Jpeg);
                                        }
                                        imgName = itemId + ".jpg";
                                    }
                                    sqltCommnd = sqltConn.CreateCommand();
                                    queryString = "INSERT INTO itemDetails (itemTitle,itemIsbn,itemAccession,itemCat,itemSubCat," +
                                        ...
                                        "imagePath) VALUES (@itemTitle,@itemIsbn,@itemAccession," +
                                        ...
                                        ",'" + entryDate + "','" + true + "','" + false + "','" + false + "','" + imgName + "');";
                                    sqltCommnd.CommandText = queryString;
                    ...
                    else
                    {
                        ...
                        for (int i = 2; i <= xlRange.Rows.Count; i++)
                        {
                            ...
                            itemTitle = Encoding.UTF8.GetString(Encoding.UTF8.GetBytes(xlRange.Cells[i, "B"].Value2 != null ? xlRange.Cells[i, "B"].Value2.ToString() : ""));
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    if (globalVarLms.sqliteData)
                                    {
                                        ...
                                        {
                                            itemImage = Image.FromFile(txtbImage.Text + @"\" + imgPath);
                                            imgName = Properties.Settings.Default.databasePath + @"\CatlogImage\" + itemId.ToUpper() + ".jpg";
                                            if (!File.Exists(imgName))
                                            {
                                                itemImage.Save(imgName, System.Drawing.Imaging.ImageFormat.Jpeg);
                                            }
                                            imgName = itemId.ToUpper() + ".jpg";
                                        }
                                        sqltCommnd = sqltConn.CreateCommand();
                                        queryString = "INSERT INTO itemDetails (itemTitle,itemIsbn,itemAccession,itemCat,itemSubCat," +
                                            "itemAuthor,itemClassification,itemSubject,rackNo,totalPages,itemPrice,addInfo1,addInfo2," +
        ...
        static String GetCellValue(WorkbookPart workbookPart, Row row, int columnIndex)
        {
            Cell cell = row.Elements<Cell>().FirstOrDefault(c => GetColumnIndex(c.CellReference) == columnIndex);
            if (cell != null)
            {
                ...
                if (cell.DataType != null && cell.DataType.Value == CellValues.SharedString)
                {
                    ...
                    if (sharedStringTablePart.SharedStringTable.Elements<SharedStringItem>().Count() > sharedStringIndex)
                    {
                        value = sharedStringTablePart.SharedStringTable.Elements<SharedStringItem>().ElementAt(sharedStringIndex).InnerText;
                    }
                }
                return value;
            }
        ...
        {
            // Parse the cell reference (e.g., "C3" => columnIndex = 3)
            int columnIndex = 0;
