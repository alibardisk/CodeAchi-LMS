    ...
    public partial class FormActivate : Form
    {
        ...
        private void btnActivate_Click(object sender, EventArgs e)
        {
            ...
            //{
                ...
                {
                    string jsonString = DecryptString(txtbSerial.Text, "mous-oma8-liba19");
                    var licenseDetails = JObject.Parse(jsonString);
                    if (macAddress != (string)licenseDetails["_mid"])
                    {
                        MessageBox.Show("Invalid Serial Key.", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information);
                        return;
                    }
                    serialKey = (string)licenseDetails["_id"];
                    int validMonth = Convert.ToInt32((string)licenseDetails["_vm"]);
                    DateTime regDate = currentDate.Date;
                    DateTime expiryDate = regDate.AddMonths(validMonth);
                    if (expiryDate < currentDate.Date)
                    {
                    ...
                    Properties.Settings.Default.expiryDate = expiryDate.Day.ToString("00") + "/" + expiryDate.Month.ToString("00") + "/" + expiryDate.Year.ToString("0000");
                    Properties.Settings.Default.licenseType = (string)licenseDetails["_vs"];
                    Properties.Settings.Default.serialKey = (string)licenseDetails["_id"];
                    Properties.Settings.Default.itemLimits = Convert.ToInt32((string)licenseDetails["_ilmt"]);
                    Properties.Settings.Default.machineLimits = 1.ToString();
                    Properties.Settings.Default.Save();
                    globalVarLms.productBlocked = false;
                    globalVarLms.expiryDate = expiryDate;
                    globalVarLms.licenseType = (string)licenseDetails["_vs"];
                    globalVarLms.opacAvailable = false;
                    globalVarLms.itemLimits = Convert.ToInt32((string)licenseDetails["_ilmt"]);
                    MessageBox.Show("Product Registered Successfully." + Environment.NewLine + "Restart the Application to use full version features…", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    this.Hide();
                    //MessageBox.Show("Please check your internet connection.", Application.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
